...
| **JWT bootstrap (settings iframe + PlatformAuthFilter)** | `CLOCKIFY_JWT_PUBLIC_KEY`, `CLOCKIFY_JWT_PUBLIC_KEY_PEM`, `CLOCKIFY_JWT_PUBLIC_KEY_MAP`, `CLOCKIFY_JWT_JWKS_URI`, `CLOCKIFY_JWT_DEFAULT_KID`, `CLOCKIFY_JWT_EXPECT_ISS`, `CLOCKIFY_JWT_EXPECT_AUD`, `CLOCKIFY_JWT_LEEWAY_SECONDS` | The shared SDK `JwtBootstrapConfig` (loaded via `RulesConfiguration`) feeds `JwtVerifier`, which prefers JWKS, then PEM map, then single PEM. `expectedAudience` defaults to the add-on key (“rules”) and `expectedIssuer` defaults to `clockify`. **Required outside dev; startup fails if omitted.** |
...
- `WorkspaceContextFilter` only activates when the shared `JwtBootstrapConfig` is present. It uses `JwtVerifier` to parse the iframe JWT, attaches workspace/user attributes, and lets controllers read them without touching `System.getenv`.
- `PlatformAuthFilter` is wired ahead of `/api/**` + `/status` so every portal call must include `Authorization: Bearer <auth_token>`. The SDK no longer trusts `workspaceId` query params outside `ENV=dev`, and the `devFromEnv()` helper remains DEV-ONLY for ad-hoc experiments.

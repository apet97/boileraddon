...
- API responses surface misconfiguration clearly: when the installation token is missing, `/api/rules`, `/api/projects`, `/api/clients`, `/api/tasks`, `/api/tags`, and explorer routes return **HTTP 412 (`RULES.MISSING_TOKEN`)** instead of leaking stack traces or 500s.
